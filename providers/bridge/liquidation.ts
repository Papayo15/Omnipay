...
export const NATIVE_RAILS: Record<string, {
  ...
  // Always "ach" as the canonical default. FedNow/Wire are set via params.rail at call time
  // (derived from BRIDGE_USE_FEDNOW / BRIDGE_USE_WIRE env vars in getRailForCountry() per-request).
...
function buildExternalAccountBody(params: CreateLiquidationParams): Record<string, unknown> {
  ...
  const ownerType = params.ownerType ?? "individual";
