...
function calcBridgeFees(amtLocal: number, localCurrency: string, fxRate: number) {
  ...
  const omniSvc = parseFloat(Math.max(usd * 0.006, 0.99).toFixed(2));
  const omniFlat = 0.49;
...
export default function P2PPage() {
  ...
  const generateLink = useCallback(async () => {
    ...
    const inTosMode   = tosModeRef.current; // capture before URL-processing resets it
    const amt = parseFloat(amountLocal);
    if (!nombre.trim() || !email.includes("@") || !account.trim() || !amt) return;
      ...
      <main className="min-h-screen bg-[#0f172a] flex flex-col px-5 pt-12 pb-10 max-w-sm mx-auto w-full">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-5 text-center">
          ...
          {shareLink && <div className="w-full space-y-3">
            {/* Compartir — abre el sheet nativo del SO */}
            <button
              onClick={async () => {
                ...
                const shareData = { title: "OmniPay — Link de pago", text: msg, url: shareLink };
              ...
              className="w-full bg-[#00C9C8] hover:bg-[#00b5b4] text-slate-900 font-bold py-4 rounded-2xl text-sm flex items-center justify-center gap-2 transition-colors"
            >
